...
/**
 * Test cases for TariffSubscription. Uses a Spring application context to
 * access autowired components.
 * 
...
public class TariffSubscriptionTests
{
  ...
  public void setUp() throws Exception
  {
    ...
    spec = new TariffSpecification(broker, PowerType.CONSUMPTION).withExpiration(baseTime.plus(TimeService.DAY * 10)).withMinDuration(TimeService.DAY * 5).addRate(new Rate().withValue(0.11));
  ...
  public void testSubscribe()
  {
    TariffSubscription sub = new TariffSubscription(customer, tariff);
    sub.subscribe(3);
    verify(mockAccounting).addTariffTransaction(TariffTransaction.Type.SIGNUP, tariff, info, 3, 0.0, 0.0);
  ...
  public void testUnsubscribe()
  {
    TariffSubscription sub = new TariffSubscription(customer, tariff);
    sub.subscribe(33);
    verify(mockAccounting).addTariffTransaction(TariffTransaction.Type.SIGNUP, tariff, info, 33, 0.0, 0.0);
    assertEquals("33 committed", 33, sub.getCustomersCommitted());
    sub.unsubscribe(8);
    verify(mockAccounting, never()).addTariffTransaction(TariffTransaction.Type.WITHDRAW, tariff, info, 8, 0.0, 0.0);
    assertEquals("25 committed", 25, sub.getCustomersCommitted());
  }

  // TODO - test subscribe and unsubscribe with non-zero signup and withdraw
  // payments
  ...
  public void testHandleRevokedTariffDefault()
  {
    ...
    TariffSpecification defaultSpec = new TariffSpecification(broker, PowerType.CONSUMPTION).addRate(new Rate().withValue(0.21));
  ...
  public void testUsePower()
  {
    ...
    verify(mockAccounting).addTariffTransaction(eq(TariffTransaction.Type.CONSUME), eq(tariff), eq(info), eq(33), eq(330.0), chargeArg.capture());
  ...
  public void testUsePowerPeriodic()
  {
    ...
    assertEquals("correct charge", 330.0 * 0.11, chargeArg.getValue(), 1e-6);
    assertEquals("correct total", 10.0, sub.getTotalUsage(), 1e-6);
    verify(mockAccounting).addTariffTransaction(eq(TariffTransaction.Type.PERIODIC), eq(tariff), eq(info), eq(33), eq(0.0), chargeArg.capture());
  ...
  public void testGetExpiredCustomerCount()
  {
    ...
    sub.subscribe(22);
    verify(mockAccounting).addTariffTransaction(TariffTransaction.Type.SIGNUP, tariff, info, 22, 0.0, 0.0);
