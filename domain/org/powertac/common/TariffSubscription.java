...
/**
 * A TariffSubscription is an entity representing an association between a
 * Customer and a Tariff. Instances of this class are not intended to be
 * serialized. You get one by calling the subscribe() method on Tariff. If there
 * is no current subscription for that Customer (which in most cases is actually
 * a population model), then a new TariffSubscription is created and returned
 * from the Tariff.
 * 
...
public class TariffSubscription
{
  ...
  /**
   * Total number of customers within a customer model that are committed to
   * this tariff subscription. This needs to be a count, otherwise tiered rates
   * cannot be applied properly.
   */
  private int customersCommitted = 0;

  /**
   * List of expiration dates. This is used only if the Tariff has a
   * minDuration, before which a subscribed Customer cannot back out without a
   * penalty. Each entry in this list is a pair [expiration-date,
   * customer-count]. New entries are added chronologically at the end of the
   * list, so the front of the list holds the oldest subscriptions - the ones
   * that can be unsubscribed soonest without penalty.
   */
  private List<ExpirationRecord> expirations;

  /**
   * Total usage so far in the current day, needed to compute charges for tiered
   * rates.
   */
  ...
  /**
   * Subscribes some number of discrete customers. This is typically some
   * portion of the population in a population model. We assume this is called
   * from Tariff, as a result of calling tariff.subscribe(). Also, we record the
   * expiration date of the tariff contract, just in case the tariff has a
   * minDuration. For the purpose of computing expiration, all contracts are
   * assumed to begin at 00:00 on the day of the subscription.
   */
  public void subscribe(int customerCount)
  {
    ...
    // if the Tariff has a minDuration, then we have to record the expiration
    // date.
    // we do this by adding an entry to end of list, or updating the entry at
    // the end.
    ...
    if (expirations.size() > 0 && expirations.get(expirations.size() - 1).getHorizon() == start.getMillis() + minDuration) {
      // update existing entry
      expirations.get(expirations.size() - 1).updateCount(customerCount);
    } else {
      // need a new entry
      expirations.add(new ExpirationRecord(start.getMillis() + minDuration, customerCount));
    }
    // }
    ...
    accountingService.addTariffTransaction(TariffTransaction.Type.SIGNUP, tariff, customer.getCustomerInfo(), customerCount, 0.0, customerCount * tariff.getSignupPayment());
  }

  /**
   * Removes customerCount customers (at most) from this subscription, posts
   * early-withdrawal fees if appropriate.
   */
  public void unsubscribe(int customerCount)
  {
    ...
    if (tariff.getEarlyWithdrawPayment() != 0.0 && penaltyCount > 0) {
      accountingService.addTariffTransaction(TariffTransaction.Type.WITHDRAW, tariff, customer.getCustomerInfo(), customerCount, 0.0, penaltyCount * tariff.getEarlyWithdrawPayment());
    }
  }

  /**
   * Handles the subscription switch in case the underlying Tariff has been
   * revoked. Returns the new subscription just in case the Tariff was revoked,
  ...
  public TariffSubscription handleRevokedTariff()
  {
    ...
    TariffSubscription result = tariffMarketService.subscribeToTariff(newTariff, customer, customersCommitted);
    log.info("Tariff " + tariff.getId() + " superseded by " + newTariff.getId() + " for " + customersCommitted + "customers");
    customersCommitted = 0;
    return result;
  }

  /**
   * Generates and returns a TariffTransaction instance for the current timeslot
   * that represents the amount of production (negative amount) or consumption
   * (positive amount), along with the credit/debit that results. Also generates
   * a separate TariffTransaction for the fixed periodic payment if it's
   * non-zero.
   */
  public void usePower(double kWh)
  {
    ...
    accountingService.addTariffTransaction(txType, tariff, customer.getCustomerInfo(), customersCommitted, kWh, customersCommitted * tariff.getUsageCharge(kWh / customersCommitted, totalUsage, true));
    ...
    if (tariff.getPeriodicPayment() != 0.0) {
      ...
      accountingService.addTariffTransaction(TariffTransaction.Type.PERIODIC, tariff, customer.getCustomerInfo(), customersCommitted, 0.0, customersCommitted * tariff.getPeriodicPayment() / 24.0);
    }
  }
